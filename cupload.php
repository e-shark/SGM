...
if ( 0 == $_FILES['fileToUpload']['size'] ) 						{ $_SESSION['uploadmessage'] = 'Error: Choose not empty file'; 									goto magain; }
if ( 0 != $_FILES['userfile']['error'] )							{ $_SESSION['uploadmessage'] = 'Error: '.$_FILES['userfile']['error'].' while uploading file';	goto magain; }
if ( !is_uploaded_file( $_FILES['fileToUpload']['tmp_name'] ) )		{ $_SESSION['uploadmessage'] = 'Error: the intrusion attempt was detected'; 					goto magain; }

if ( move_uploaded_file( $_FILES['fileToUpload']['tmp_name'], $uploadfile ) ) { 
	if( FALSE == exec( FWUPDATE_SCRIPT, $output, $retv ) ) { $_SESSION['uploadmessage'] = 'Error: failed while updating the firmware';	goto magain; }
	if( $retv != 0 ) { $_SESSION['uploadmessage'] = 'Error: '.$retv.' while updating the firmware';	goto magain; }
